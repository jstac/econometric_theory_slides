...
# Vector coordinates
x_coords = np.array((xvec[0], yvec[0]))
y_coords = np.array((xvec[1], yvec[1]))
z_coords = np.array((xvec[2], yvec[2]))
...
if 0:
    ...
    # Then draw extended line
    s = 3
    xx = (-s * x_coords[i], s * x_coords[i])
    yy = (-s * y_coords[i], s * y_coords[i])
    zz = (-s * z_coords[i], s * z_coords[i])
    ax.plot(xx, yy, zz, 'g-', lw=2, alpha=0.5)
